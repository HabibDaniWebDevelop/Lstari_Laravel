<table class="table table-borderless table-sm text-center" id="tabelForm" style="vertical-align: top">
    ...
    <tbody>
        @for ($i = 0; $i < $jumlahMatras; $i++)
        <tr>
            <td>{{$i+1}}</td>
            <td>
                <select name="product" id="product_{{$i+1}}" class="form-select product">
                    @foreach ($BBGCProduct as $item)
                    <option value="{{$item->ID}}">{{$item->SWProduct}}</option>
                    @endforeach
                </select>
            </td>
            <td>
                <select name="tipeMatras" id="tipeMatras_{{$i+1}}" class="form-select tipeMatras">
                    <option value="Samir">Samir</option>
                    <option value="Roll">Roll</option>
                </select>
            </td>
            <td><input type="file" class="form-control fileAutocad" name="fileAutocad_{{$i+1}}" id="fileAutocad_{{$i+1}}"></td>
            <td><input type="file" class="form-control fotoGambarTeknik" name="fotoGambarTeknik_{{$i+1}}" id="fotoGambarTeknik_{{$i+1}}"><input type="hidden" class="form-control fotoGambarTeknik64" name="fotoGambarTeknik64_{{$i+1}}" id="fotoGambarTeknik64_{{$i+1}}"></td>
            <td>
                <table class="table table-borderless table-sm text-center" id="tabelBahanBaku_{{$i+1}}">
                    <thead class="table-secondary">
                        <tr>
                            <th>Nama Bahan</th>
                            <th width="20%">Qty</th>
                    ...
                    <tbody>
                        <tr class="rowBahanBaku rowBahanBaku_{{$i+1}}" id="rowBahanBaku_{{$i+1}}_1">
                            <td>
                                <select name="namaBahan_{{$i+1}}_1" id="namaBahan_{{$i+1}}_1" class="form-select namaBahan">
                                    @foreach ($rawMaterial as $item)
                                        <option value="{{$item->ID}}">{{$item->Name}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td><input type="number" min="1" max="10" name="qtyBahan_{{$i+1}}_1" id="qtyBahan_{{$i+1}}_1" class="form-control qtyBahan"></td>
                            <td>
                                <button onclick="newRow({{$i+1}})" class="btn btn-primary btn_add_row">+</button> <button onclick="removeRow2({{$i+1}},1)" class="btn btn-primary btn_remove">-</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
