...
<ol class="breadcrumb sm-2 mb-1">
    ...
    <li class="breadcrumb-item active">Lilin Stone Cast</li>
</ol>
@endsection
...
<div class="row">
    <div class="col-md-12">
        <div class="card mb-4">
            ...
            @include('Produksi.Lilin.LilinStoneCast.data')
...
function JumlahBatu(Batu, no) {
    var Batu = Batu;
    console.log(Batu);
    var Batus = $('#tabel1').find('.Batu')

    var total = 0;
    for (let i = 0; i < Batus.length; i++) {
        var aa = parseInt($(Batus[i]).val())

        total = total + aa
    }
    console.log(total);
    $('#Jumlah_Batu').text(total);
}
...
function SearchWaxInjectOrder() {
    ...
    $.ajax({
        type: "GET",
        url: "/Produksi/Lilin/LilinStoneCast/Items/" + IDSPKOInject,
        dataType: 'json',
        beforeSend: function() {
            $(".preloader").show();
        },
        ...
        success: function(data) {
            ...
            data.data.items.forEach(function(value, i) {
                ...
                let Batu =
                    ...
                    no + "' value='0' onkeyup='JumlahBatu(this.value," + no +
                    ")' tabindex='6'></td>"
                let Keterangan =
                    "<td><input type='text' class='form-control form-control-sm fs-6 w-100 text-center Keterangan' id='Keterangan_" +
                    no + "' value='' tabindex='6'></td>"
                let Action =
                    "<td align='center'><button disabled class='btn btn-info btn-sm add' tabindex='6' type='button' onclick='add(\"" +
                    value.WorkOrder + "\"," + value.IDWorkOrder + "," + no + "," + value.Qty +
                    ")' id='add_" + no +
                    "'><i class='fa fa-plus'></i></button>&nbsp;&nbsp;&nbsp;<button type='button' tabindex='6' class='btn btn-danger btn-sm remove' onclick = 'remove(" +
                    no + ")' id='remove_" + no +
...
function addbloc(WorkOrder, IDWorkOrder, id, Qty) {
...
function Simpan() {
    ...
    //!  ------------------------    Check if have items     ------------------------ !!
    if (Batus.length === 0 || IDProducts.length === 0) {
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
    ...
    $.ajax({
        type: "POST",
        url: "/Produksi/Lilin/LilinStoneCast/Simpan",
        data: data,
        dataType: 'json',
        beforeSend: function() {
            $(".preloader").show();
...
function isioperator() { // input form id operator untuk trigger form nama operator
    ...
    if (IdOperator !== '') {
        $.get('/Produksi/Lilin/LilinStoneCast/Operator/' + IdOperator, function(data) {
            $('#NamaOperator').val(data.namaop);
        });
    }
...
function Ubah() {
    ...
    $.ajax({
        type: "PUT",
        url: "/Produksi/Lilin/LilinStoneCast/Update",
        data: data,
        dataType: 'json',
        beforeSend: function() {
            $(".preloader").show();
...
function Search() {
    ...
    $.ajax({
        type: "GET",
        url: "/Produksi/Lilin/LilinStoneCast/Search?keyword=" + cari,
        dataType: 'json',
        beforeSend: function() {
            $(".preloader").show();
        },
        ...
        success: function(data) {
            ...
            data.data.items.forEach(function(value, i) {
                ...
                let Batu =
                    "<td><input type='number' class='form-control form-control-sm fs-6 w-100 text-center Batu' id='Batu_" +
                    no + "' value='" + value.Batu + "' onkeyup='JumlahBatu(this.value," + no +
                    ")' tabindex='6'></td>"
                let Keterangan =
                    "<td><input type='text' class='form-control form-control-sm fs-6 w-100 text-center Keterangan' id='Keterangan_" +
                    no + "' value='" + value.Keterangan + "' tabindex='6'></td>"
                let Action =
                    "<td align='center'><button disabled class='btn btn-info btn-sm add' type='button' onclick='add(\"" +
                    value.WorkOrder + "\"," + value.IDWorkOrder + "," + no + "," + value.Qty +
                    ")' id='add_" + no +
