<div class="card mx-auto">
    <ul class="nav nav-pills btn-group" role="group">
        ...
        <li class="nav-item col mx-auto">
            <button type="radio" class="nav-link active" role="tab" data-bs-toggle="tab" onclick="ShowListItemDC()"
            ...
            <table class="table table-border table-hover table-sm rounded-4" id="tabel1">
                <thead class="table-secondary sticky-top zindex-2 rounded-4">
                    <tr style="text-align: center">
                        ...
                        <th width="10%">Tok</th>
                        <th width="10%">SC</th>
                        <th colspan="2">Rekap Lilin (RL)</th>
                        <th>action</th>
                    </tr>
                </thead>
                <tfoot>
                ...
                {{-- {{ dd($query); }} --}}
                <tbody>
                    @forelse ($tambahitemSPKO as $item)
                    <tr id="baris" style="text-align: center">
                        ...
                        <td class="m-0 p-0">
                            <input style="text-align: center" type="text"
                                class="Qty form-control form-control-sm fs-6 w-20" name="Qty[]"
                        ...
                        <td class="m-0 p-0">
                            <input style="text-align: center" type="text"
                                class="Sc form-control form-control-sm fs-6 w-100" name="Sc[]"
                                id="Sc{{ $loop->iteration }}" value="" placeholder="Harap diisi">
                        </td>
                        <td>{{ $item->waxorder }}
                        ...
                        <td><button type='button' class='btn btn-danger btn-sm' onclick='remove(this.value)'
                                value='{{ $loop->iteration }}' id='remove_{{ $loop->iteration }}'><i
    ...
    <div class="tab-pane fade" id="tabelKaret" role="tabpanel">
        <div class="table-responsive text-nowrap rounded-4" style="height:calc(100vh - 490px);">
            <table class="table table-border table-hover table-sm rounded-4" id="tabel6">
                ...
                <tbody>
                    @forelse ($tambahkaretSPKO as $tkp)
                    <tr class="klik2" id="{{ $tkp->ID }}">
                        <td>
                            <input class="form-check-input pilihkaret" type="checkbox" name="id[]"
                                id="cek_{{ $tkp->ID }}" value="{{  $tkp->ID }}" data-lokasi="{{$tkp->lokasi}}" disabled>
                        </td>
