                    ...
                    <tr id="baris" style="text-align: center">
                        ...
                        <td class="m-0 p-0">
                            <input style="text-align: center" type="number"
                                ...
                                id="Qty{{ $loop->iteration }}" onkeyup="jumlahqty(this.value)" value="{{ $item->Qty}}">
                        </td>
                        <td class="m-0 p-0">
                        ...
                        <td align='center'><button class='btn btn-info btn-sm' type='button'
                                onclick='add("{{ $loop->iteration }}","{{ $item->SWUsed }}","{{ $item->Qty}}")'
                                id='add_{{ $loop->iteration }}'><i
                                    class='fa fa-plus'></i></button>&nbsp;&nbsp;&nbsp;<button type='button'
                                class='btn btn-danger btn-sm' onclick='remove("{{ $loop->iteration }}")'
                                id='remove_{{ $loop->iteration }}'><i class='fa fa-minus'></i></button></td>
                        ...
                        <td hidden><input type="hidden" class="purposeitem form-control form-control-sm fs-6 w-100"
                                name="purposeitem[]" id="purposeitem{{ $loop->iteration }}" value="A">
                        </td>
                    </tr>
                    @empty
        ...
        <div class="table-responsive text-nowrap rounded-4" style="height:calc(100vh);">
            <table class="table table-border table-hover table-sm rounded-4" id="tabel6">
                ...
                <tbody>
                    ...
                    <tr class="klik2" id="{{ $loop->iteration }}">
                        <td>
                            <input class="form-check-input karet" type="checkbox" name="id[]"
                                id="cek_{{ $loop->iteration }}" value="{{  $tkp->ID }}" data-lokasi="{{$tkp->lokasi}}"
                                disabled>
                        </td>
                        ...
                        <td>{{ $tkp->lokasi }}</td>
                        <td>{{ $tkp->Active }}</td>
                        <td> <button type="button" class="btn btn-info" id="lihat_{{ $loop->iteration }}"
                                value="{{ $tkp->ID }}" onclick="lihat(this.value)">
                                <span class="fas fa-camera"></span></button></td>
