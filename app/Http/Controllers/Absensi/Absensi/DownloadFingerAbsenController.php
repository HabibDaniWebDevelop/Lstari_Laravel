...
class DownloadFingerAbsenController extends Controller{
    ...
    public function index(Request $request){
        ...
        $datenow = date("Y-m-d");
        return view('Absensi.Absensi.DownloadFingerAbsen.index', compact('datenow'));
    }
    ...
    public function KlikCheck(Request $request){
        ...
        $reader = Reader::createFromFileObject($file->openFile())->setHeaderOffset(0);
        // Loop through the CSV file for cleaning data
        foreach ($reader as $index => $row) {
            $temp_excel_data = [];
            foreach ($row as $key => $value) {
                $temp_excel_data[trim(preg_replace('/\t+/', '', $key))] = trim(preg_replace('/\t+/', '', $value));
            }
            if ($temp_excel_data['Personnel No.'] != "--") {
                $cleaned_data[] = $temp_excel_data;
            }
        }

        foreach ($cleaned_data as $key => $value) {
            $temp = [];
            $temp['Employee'] = intval($value['Personnel No.']);
            $transaction_date = $value['Time'];
            // Convert $transaction_date to datetime
            $transaction_date = explode(" ", $transaction_date);
            $temp['TransDate'] = $transaction_date[0];
            $temp['TransTime'] = $transaction_date[1];
            $temp['Status'] = strtotime($transaction_date[1]) >= strtotime("13:00:00") ? "K" : "M";
            $temp['Machine'] = explode(" ", $value['Event point'])[1];
            
            // Check if data is already in accesscontroltemp
            $check_data = accesscontroltemp::where('Employee', '=', $temp['Employee'])->where('TransDate', '=', $temp['TransDate'])->where('TransTime', '=', $temp['TransTime'])->first();
    ...
    // Function for saving absent to accesscontroltemp
    public function SaveAbsent(Request $request){
        ...
        // insert cleaned data to accesscontroltemp
        foreach ($cleaned_data as $key => $value) {
            $temp = [];
            $temp['Employee'] = intval($value['Personnel No.']);
            // Convert $transaction_date to datetime
            $transaction_date = explode(" ", $value['Time']);
            $temp['TransDate'] = $transaction_date[0];
            $temp['TransTime'] = $transaction_date[1];
            $temp['Status'] = strtotime($transaction_date[1]) >= strtotime("13:00:00") ? "K" : "M";
            $temp['Machine'] = explode(" ", $value['Event point'])[1];
    ...
    // Function for posting data accesscontroltemp to workhour, absent, absentitem, checkclock
    public function PostingFingerAbsent(){
